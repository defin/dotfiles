...
;; The standard functions BACKWARDS-SEXP and friends don't ignore comments like
;; they should.  Too bad.  That means we have to do it ourselves.
...
;; Another little function that returns the position of the car of the list that
;; contains the given pos.
(defun car-of-enclosing-list (pos)
  (save-excursion
    (goto-char pos)
    (backward-up-list)
    (current-pos)))
...
(put 'define-method-combination 'common-lisp-indent-hook
...
(put 'generic-function 'common-lisp-indent-hook 'lisp-indent-generic-function)

(put 'generic-labels 'common-lisp-indent-hook 'lisp-indent-generic-flet)

(put 'symbol-macrolet 'common-lisp-indent-hook 1)

(put 'with-accessors 'common-lisp-indent-hook
...
(put 'with-added-methods 'common-lisp-indent-hook 'lisp-indent-with-added-methods)

(put 'with-slots 'common-lisp-indent-hook
     '((&whole 6 &rest (&whole 1 1 2)) 4 &body))


;; These aren't really a part of CLOS, but they often appear in PCL programs.

(put 'defconstructor 'common-lisp-indent-hook 3)
